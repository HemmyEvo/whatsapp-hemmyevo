...
export default function VideoUIKit() {
  ...
  const username = user?.username || (user?.emailAddresses[0]?.emailAddress.split("@")[0] || "Guest");

  if (!userId) {
    // Handle the case where user or userId is not available (e.g., show an error or fallback)
  ...
  return <div className='myCallContainer' ref={myMeeting} style={{ width: "100%", height: "100%" }}></div>;
}
